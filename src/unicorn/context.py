...
class GCodeContext:
    def __init__(self, xy_feedrate, z_feedrate, start_delay, stop_delay, pen_up_angle, pen_down_angle, z_height, finished_height, x_home, y_home, register_pen, num_pages, continuous, file):
      ...
      self.preamble = [
        ...
        "G21 ;(metric ftw)",
        "G90 ;(absolute mode)",
        "G92 X%.2f Y%.2f Z%.2f ;(you are here)" % (self.y_home, self.x_home, self.z_height),
        ""
      ]
	...
	"M400 ;(Finish all moves)",
        ...
        "G90 ;(absolute mode)",
        "G0 X%0.2f Y%0.2f F%0.2f" % (self.y_home, self.x_home, self.xy_feedrate),
    ...
    def start(self):
      self.codes.append("M400 ;(Finish all moves)")
    ...
    def go_to_point(self, x, y, stop=False):
      ...
      if stop:
        ...
        if self.drawing: 
            ...
            self.codes.append("G4 P%d ;(wait %dms)" % (self.stop_delay, self.stop_delay))
            self.drawing = False
        self.codes.append("G1 X%.2f Y%.2f F%.2f" % (y,x, self.xy_feedrate))
      self.last = (x,y)
    ...
    def draw_to_point(self, x, y, stop=False):
      ...
      if stop:
        ...
        if self.drawing == False:
            ...
            self.codes.append("G4 P%d ;(wait %dms)" % (self.start_delay, self.start_delay))
            self.drawing = True
        self.codes.append("G1 X%0.2f Y%0.2f F%0.2f" % (y,x, self.xy_feedrate))
      self.last = (x,y)
